...
{
    // Получаем общее количество точек миссии
    uint16_t total_cmds = plane.mission.num_commands();

    // Проверяем, есть ли точки в миссии
    if (total_cmds == 0) {
        gcs().send_text(MAV_SEVERITY_WARNING, "Mission has no waypoints.");
        return;
    }

    // Получаем последнюю точку миссии
    AP_Mission::Mission_Command last_cmd;
    if (!plane.mission.read_cmd_from_storage(total_cmds - 1, last_cmd)) {
        gcs().send_text(MAV_SEVERITY_ERROR, "Failed to read last waypoint.");
        return;
    }

    // Убедимся, что это команда NAV_WAYPOINT
    if (last_cmd.id != MAV_CMD_NAV_WAYPOINT) {
        gcs().send_text(MAV_SEVERITY_WARNING, "Last command is not a NAV_WAYPOINT.");
        return;
    }

    // Вызываем функцию do_nav_wp для полета к последней точке
    do_nav_wp(last_cmd);
    gcs().send_text(MAV_SEVERITY_INFO, "Flying to last waypoint.");
}
...
//    gcs().send_text(MAV_SEVERITY_INFO, "Normal pitch");
